...
struct AppUser: Identifiable, Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if photoURL == nil {
            // Backwards compatibility: some docs may use "photoUrl" (lowercase L).
            ...
            let dc = try decoder.container(keyedBy: DynamicKey.self)
            if let alt = try dc.decodeIfPresent(String.self, forKey: DynamicKey(stringValue: "photoUrl")!) {
                photoURL = alt
            }
        }
