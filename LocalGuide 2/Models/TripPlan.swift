...
struct TripPlan: Codable, Identifiable {
    ...
    /// Optional in-app recommendations linked to this plan.
    /// These are Firestore document ids from `tours` and `experiences`.
    var recommendedTourIds: [String]? = nil
    var recommendedExperienceIds: [String]? = nil
    var createdAt: Date?

    /// Convenience: display title derived from the AI plan (if present).
    var planTitleFallback: String {
        if let t = plan["title"]?.value as? String, !t.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return t
        }
        return "\(city), \(country)"
    }
